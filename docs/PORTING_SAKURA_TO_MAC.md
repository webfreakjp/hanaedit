...
2. Sakura の移植可能な機能を platform-neutral な interface の後ろに移す。
   - 文字コード検出と変換
   - 検索と置換
   - grep のファイル列挙
   - タイプ別設定
...
`macos/SakuraMac` の初期版では、小さくても実際に使えるエディタを先に作っています。
...
Sakura の Win32 view stack を最初から丸ごと書き換えるのではなく、起動できる
