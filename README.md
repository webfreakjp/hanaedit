...
- 空白・タブ・改行コードのビジュアル表示切り替え: `Option-Command-I`

検索、置換、grep では `Regular expression` をオンにすると正規表現検索になります。
正規表現の置換では `$1`、`$2` のようなキャプチャ参照を使えます。
置換欄では `\t`、`\n`、`\r`、`\\` を、それぞれタブ、改行、復帰、バックスラッシュ
として展開します。
空白・タブ・改行コードのビジュアル表示をオンにすると、半角スペースは `·`、タブは
`→`、改行コードは `LF` / `CRLF` / `CR` として表示します。
